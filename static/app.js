...
async function loadIncidentBanner(){
  ...
  el.onclick = () => showPage('incidentes');
}
...
document.getElementById('formTarget').addEventListener('submit', async (e) => {
  ...
  if(!r.ok){ const j = await r.json(); alert(j.detail || 'Erro ao adicionar'); return; }
  e.target.reset();
  await loadSettings();
});
...
document.getElementById('formIntervals').addEventListener('submit', async (e) => {
  ...
  alert('Intervalos salvos. Passam a valer no próximo ciclo de cada worker.');
});
...
document.getElementById('testTelegramBtn').addEventListener('click', async () => {
  ...
  if(r.ok){ alert('Mensagem de teste enviada! Confira o Telegram.'); }
  else { const j = await r.json(); alert('Erro: ' + (j.detail || 'não foi possível enviar')); }
});
